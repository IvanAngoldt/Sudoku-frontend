...
const Avatar = forwardRef(({ isEditing = false, userID }, ref) => {
  ...
  // Загружаем аватар при изменении userID или avatarVersion
  useEffect(() => {
    const fetchAvatar = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8080/users/${userID}/avatar?v=${avatarVersion}`,
          {
  ...
  // Освобождаем старую avatar blob ссылку
  useEffect(() => {
    return () => {
  ...
  // Освобождаем превью после удаления
  useEffect(() => {
    return () => {
